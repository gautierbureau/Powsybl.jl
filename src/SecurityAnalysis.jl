...
module SecurityAnalysis
  ...
  """
      add_phase_tap_changer_position_action(analysis, action_id, transformer_id, tap_position;
                                            is_relative = false, side = SIDE_NONE)

  Add a remedial action setting the tap position of a transformer's phase tap changer.
  `side` selects the leg of a three windings transformer (`SIDE_NONE` for a two
  windings transformer).
  """
  function add_phase_tap_changer_position_action(analysis::SecurityAnalysisContext, action_id::String,
                                                 transformer_id::String, tap_position::Integer;
                                                 is_relative::Bool = false, side::Side = SIDE_NONE)
  ...
  """
      add_ratio_tap_changer_position_action(analysis, action_id, transformer_id, tap_position;
                                            is_relative = false, side = SIDE_NONE)
  ...
  """
      add_shunt_compensator_position_action(analysis, action_id, shunt_id, section_count)
  ...
  function add_shunt_compensator_position_action(analysis::SecurityAnalysisContext, action_id::String,
                                                 shunt_id::String, section_count::Integer)
    LibPowsybl.add_shunt_compensator_position_action(analysis.handle, action_id, shunt_id, Cint(section_count))
    return nothing
  end
